...
def foodHeuristic(state, problem):
    ...
    if not food_list:
        return 0         
    node = (0,0), (0,0), 0    # current_food, next_food, cost
    #capsules = list(capsules)
    for current_food in food_list:
        for next_food in food_list:
            cost = eucledian_distance(current_food, next_food)
            if node[2] < cost:
                node = (current_food, next_food, cost)
    food_list.sort()
    heuristic = 0   
    if node[0] == (0,0) and cost == 0: 
        heuristic = eucledian_distance(position,food_list[0])
    else: 
        heuristic = manhattan_distance(position,node[0])   
    return heuristic
